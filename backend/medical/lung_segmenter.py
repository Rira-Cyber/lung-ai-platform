...
class LungSegmenter:
    ...
    def segment_slice(self, image: np.ndarray) -> np.ndarray:
        ...
        # Step 2
        inside_body = self._remove_outside_air(image, body_mask)

        # Step 3
        lung_mask = self._extract_lungs(inside_body)
    ...
    def segment_volume(self, volume: np.ndarray) -> np.ndarray:
        ...
        for slice_image in volume:
            masks.append(self.segment_slice(slice_image))
    ...
    def _extract_body(self, image: np.ndarray) -> np.ndarray:
        body = Threshold.body(image, self.body_threshold)

        body = Morphology.close(body, kernel=7)

        body = Morphology.fill(body)

        body = ConnectedComponents.largest(body)
    ...
    def _extract_lungs(self, image: np.ndarray) -> np.ndarray:
        lungs = Threshold.lung(image, self.lung_threshold)

        lungs = ConnectedComponents.largest_n(lungs, n=2)

        lungs = Morphology.open(lungs, kernel=3)

        lungs = Morphology.close(lungs, kernel=5)

        lungs = Morphology.fill(lungs)
